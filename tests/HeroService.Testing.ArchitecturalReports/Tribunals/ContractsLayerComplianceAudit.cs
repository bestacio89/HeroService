...
{
  /// <summary>
  /// ?? HeroService Tribunal — Contracts Layer Governance
  /// Enforces naming, immutability, and lifetime conventions for the Contracts layer.
  /// Dynamically adapts to any solution prefix (HeroService, Raanz, etc.).
  /// </summary>
  public sealed class ContractsLayerComplianceAudit : ArchitecturalAuditBase
  {
    ...
    public void Contracts_Governance()
    {
      ExecuteTribunal("Contracts Layer Compliance Audit", (sb, markViolation) =>
      {
        ...
        sb.AppendLine("---------------------------------------------------------------");
        sb.AppendLine($"???  {prefix}.Contracts Audit Verdict: Excellent");
        sb.AppendLine("??  DTOs: Immutable ?  |  Commands/Queries: Properly Named ?| Repositories: properly Scoped and Independent from framework provided notions");
      });
    }
