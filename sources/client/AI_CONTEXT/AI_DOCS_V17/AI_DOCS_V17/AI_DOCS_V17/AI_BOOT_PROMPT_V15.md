# AI_BOOT_PROMPT_V15 — Unity MOBA Prototype

> Version 15 — Minimap V1.1 validée, socle GameManager toujours stable, prochaine priorité court terme : Hero Selection Screen.
...
- style inspiration: Mobile Legends, but not as a copy target

## Canonical validated state [V15]
### Spawn / local player
...
### Minimap [newly validated in V15]
- `MinimapBounds` exists
- `MinimapPresenter` exists
- `MinimapMarkerView` exists
- a `MinimapPanel` exists in the Canvas
- `MarkersRoot` exists under the minimap panel
- a `MinimapSystem` scene object exists outside the Canvas
- local player marker is displayed
- ally and enemy markers are displayed
- world-to-UI projection is functional
- marker sizes were improved for readability
- marker clamp inside the minimap is implemented
- local player marker rotation is validated
- a triangle sprite is used for the local player marker
- the minimap stays a passive view layer
...
- Do not place gameplay authority inside minimap scripts
...
- Do not claim advanced map innovation is a short-term committed production task; it is only a documented future design lane
- Start the next feature from the stabilized V15 baseline, not from an earlier partial state
